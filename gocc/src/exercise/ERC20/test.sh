...
# Instantiate
CC_CONSTRUCTOR='{"Args":["init","RakmoToken","10000", "This is my new token!","Rakmo"]}'
chain_instantiate

# Get the balance for bankA
set_test_case   'Chaincode Should be initialized as "Bank_A" with 10000 tokens'

CC_QUERY_ARGS='{"Args":["balanceOf","Bank_A"]}' 
chain_query
assert_json_equal "$QUERY_RESULT"  '.response.balance'  "10000"

set_test_case   'Transfer 4000 tokens from Bank_A to Bank_B - balance for Bank_A should be 6000'
# Invoke the transfer
export CC_INVOKE_ARGS='{"Args":["transfer", "Bank_A", "Bank_B", "4000"]}'
chain_invoke 


# Get the balance for Bank_A
CC_QUERY_ARGS='{"Args":["balanceOf","Bank_A"]}' 
chain_query
assert_json_equal "$QUERY_RESULT"  '.response.balance'  "6000"

set_test_case   'Bank_B balance should be 4000'
CC_QUERY_ARGS='{"Args":["balanceOf","Bank_B"]}' 
chain_query
assert_json_equal "$QUERY_RESULT"  '.response.balance'  "4000"
