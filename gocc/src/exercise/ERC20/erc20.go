...
// TransactionPrefix is used for creating the key for transactions
...
func (token *ERC20TokenChaincode) Invoke(stub shim.ChaincodeStubInterface) peer.Response {
	...
	case	function == "setTransaction":
			return setTransaction(stub, args)
	case	function == "getTransaction":
			return getTransaction(stub, args)
	}
...
/**
 * Getter function
 * function getTransaction(address transactionId) public view returns (string status);
 * Returns the status for the specified transactionId (from%to%uuid)
 * {"Args":["getTransaction","transaction-id"]}
 **/
 func getTransaction(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	// Check if transaction id is in the arguments
	if len(args) < 1   {
		return errorResponse("Needs TransactionId!!!", 6)
	}
	TransactionId := args[0]
	bytes, err := stub.GetState(TransactionPrefix+TransactionId)
	if err != nil {
		return errorResponse(err.Error(), 7)
	}

	response := transactionJSON(TransactionId, string(bytes))
 ...
 /**
  * Setter function
  * function setTransaction(address from, address to, uint amount, string status) public returns (bool success);
  * set transaction status 
  * {"Args":["from","to","amount","status"]}
  **/
  func setTransaction(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	// Check if owner id is in the arguments
	if len(args) < 4    {
		return errorResponse("Needs from, to, amount and status!!!", 700)
	}
	...
	//set the transaction
	//! to add status in value
	bytes := []byte(from+"+"+to+"+"+strconv.FormatInt(int64(amount), 10)+"+"+status+"+"+time.Now().Format(time.RFC3339)+"]")
	fmt.Println(TransactionPrefix+from+to)
	fmt.Println(bytes)

	id := uuid.New()

	err = stub.PutState(TransactionPrefix+id.String(), bytes)
	// Emit Transaction Event
	eventPayload := "{\"from\":\""+from+"\", \"to\":\""+to+"\",\"amount\":"+strconv.FormatInt(int64(amount),10)+",\"status\":\""+status+"\",\"Transaction id\": \""+id.String()+"\"}"
	stub.SetEvent("setTransaction", []byte(eventPayload))
	return successResponse("Transaction stored successfully for transaction id: "+ id.String());
  }
 ...
 // transactionJSON creates a JSON for representing the transaction
 func transactionJSON(TransactionId, status string) (string) {
