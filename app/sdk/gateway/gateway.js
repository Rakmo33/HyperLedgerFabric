...
app.get('/login', (req, res) => {
  const execResult = shell.exec('npm run initAdmin -- acme Admin');
  console.log(execResult);
  if (execResult.code !== 0) {
    console.log(err);
    res.status(300).send('Bank Login failed!');
  } else {
    res.status(200).send('Bank Login successful!');
  }
});
...
app.get('/api/transactions/:id', async (req, res) => {
  await queryContract(req.params.id).then((str) => {
    ...
    const details = str
      ?.slice(detailsStartIndex, detailsEndIndex - 1)
      ?.split('+');
